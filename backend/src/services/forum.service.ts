...
export class ForumService {
  ...
  async createThread(data: {
    ...
    if (
      userRows[0]?.restricted_until &&
      new Date(userRows[0].restricted_until) > new Date()
    ) {
      throw new AppError(
        403,
        `Tu participación en el foro está temporalmente restringida hasta ${new Date(userRows[0].restricted_until).toLocaleString()}`,
      );
    }
  ...
  async createReply(data: {
    ...
    if (thread[0].is_closed) {
      throw new AppError(
        400,
        'Este debate ha sido cerrado por el docente y no admite nuevas respuestas',
      );
    }
  ...
  async voteReply(replyId: number, action: 'vote' | 'unvote' = 'vote') {
    ...
    return {
      message: action === 'unvote' ? 'Voto retirado' : 'Voto registrado',
    };
  }
  ...
  async deleteReply(replyId: number, userId: number, userRole: string) {
    ...
    if (userRole === 'admin' || userRole === 'moderator') {
      await logAuditAction({
        ...
        userRole,
        action:
          userRole === 'admin' ? 'ADMIN_DELETE_REPLY' : 'MODERATE_DELETE_REPLY',
  ...
  async deleteThread(threadId: number, userId: number, userRole: string) {
    ...
    await pool.query(
      'UPDATE forum_threads SET is_active = FALSE WHERE id = ?',
      [threadId],
    );
